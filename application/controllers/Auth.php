...
class Auth extends My_Controller
{
	    ...
	    if (isset($_POST['username']) && isset($_POST['password'])) {
	        ...
	        if ($user) {
	            if ($user->deleted_at) {
    	            ...
    	            $this->session->set_userdata('fullname', $user->name);
	...
	private function save_remember_hash($user_id, $remember)
	{
		if ($remember) {
			...
			$this->db->insert('remember_users', array(
				'user_id' => $user_id,
				'browser' => $browser,
				'remember_hash' => $hash,
				'created_at' => time()
	...
	public function manage()
	{
	    ...
	    $this->render('auth/manage', array('report_enable' => $user->report_enable));
	}
	...
	public function admin_password()
	{
	        ...
	        if ($_POST['new_password'] == $_POST['new_re_password']) {
    	        ...
    	        if ($user) {
        	        if (!$user->admin_password) {
        	            $this->user_model->update($user->id, array('admin_password' => md5($_POST['new_password'])));
	    ...
	    $this->render('auth/admin_password', array('message' => $message, 'err_message' => $err_message));
	}
